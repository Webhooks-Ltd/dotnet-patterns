...
> **Ref:** `FTR001` | **Category:** Feature-oriented
...
- The domain model is the primary asset — rich entities with shared invariants spanning multiple features need a domain-centric pattern (LYR003 or DOM001)
...
- Small CRUD apps — LYR001 is simpler and you don't have enough features to justify the per-feature overhead
...
There are no layers to traverse. The handler is the feature. It queries what it needs, does what it needs to do, and returns the result. Compare this with LYR003 where the same operation crosses Controller → MediatR → Handler → Repository → DbContext.
...
**The escape hatch:** If you find yourself building a rich shared domain model with entities calling each other and enforcing cross-entity invariants, you've outgrown this pattern. Move to LYR003 or DOM001.
...
6. **Not knowing when to stop.** If every new feature requires modifying shared domain logic, entities are deeply interrelated, or you're building complex invariants that span features — this pattern isn't working. Graduate to LYR003 or DOM001.
