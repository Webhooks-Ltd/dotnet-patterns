...
- You've outgrown N-Tier (LYR001): service classes are getting large, business logic is leaking into controllers or repositories
...
- Pure CRUD with no business rules — use N-Tier (LYR001), it's simpler and you won't benefit from the extra structure
- Large teams or complex domains where you need **compile-time** enforcement of layer boundaries — use Full Clean Architecture (LYR003)
- Multiple deployment targets (API + worker + CLI) sharing domain logic — multiple projects (LYR003) make this cleaner
...
The key difference from N-Tier (LYR001): the `Order` entity itself contains business logic (e.g., `order.AddItem(product, quantity)` validates stock), rather than the service doing everything.
...
1. **Anaemic domain entities.** Entities with only properties and no methods are DTOs, not domain objects. If `Order` has public setters and no behaviour methods, you're doing N-Tier with extra folders. Either add real behaviour or drop back to LYR001.
...
8. **Over-engineering for a CRUD app.** If your entities have no behaviour methods and your use cases are all just "get from repo, map, return," you don't need this pattern. Use LYR001 and save yourself the ceremony.
