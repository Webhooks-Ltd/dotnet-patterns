...
- You need **compile-time enforcement** — convention-based boundaries (LYR002) aren't holding up as the team grows
...
- Solo developer or small team (1–3) — use LYR002, you don't need the compiler to enforce discipline you can see
...
1. **Anaemic domain model.** Entities with only public properties and no methods. If `Order` has `public OrderStatus Status { get; set; }` with no `Submit()` method enforcing rules, you've built four projects of indirection around a CRUD app. Either add real behaviour or use LYR001.
